...
else ifeq ($(findstring distcc,$(CC)),distcc)
CLANG_VERSION    := $(shell $(TOOLCHAIN)$(subst @,,$(CC)) cc --version  2> /dev/null | head -n 1 | grep -e 'version [0-9]\+\.[0-9]\+\(\.[0-9]\+\)\?' -o | grep -e '[0-9]\+\.[0-9]\+\(\.[0-9]\+\)\?' -o | tail -n 1)
else
