...
```sh
make cmake OSD="headless" verbose=1 TARGETOS="macosx" CONFIG="headless-dbg" TARGET=mame SUBTARGET=tiny MACOSX_DEPLOYMENT_TARGET=10.14 -j8
```
...
add_custom_command(
  ...
  DEPENDS ${CMAKE_CURRENT_SOURCE_DIR}/../../../../../../src/mame/tiny.lst ${CMAKE_CURRENT_SOURCE_DIR}/../../../../../../scripts/build/makedep.py
  COMMAND python ${CMAKE_CURRENT_SOURCE_DIR}/../../../../../../scripts/build/makedep.py  driverlist ${CMAKE_CURRENT_SOURCE_DIR}/../../../../../../src/mame/tiny.lst  > ${CMAKE_CURRENT_SOURCE_DIR}/../../../../../generated/mame/tiny/drivlist.cpp
  COMMENT "@echo Building driver list..."
)

if(CMAKE_BUILD_TYPE MATCHES "Debug")
  add_executable(mametinyd ${source_list})
  target_link_libraries(mametinyd mame_tiny frontend optional emu formats dasm netlist utils expat softfloat softfloat3 wdlfft jpeg 7z asmjit lua lualibs linenoise zlib flac utf8proc sqlite3 ocore_headless -lm -lpthread -lutil -framework Foundation)
endif()

if(CMAKE_BUILD_TYPE MATCHES "Release")
  add_executable(mametiny ${source_list})
  target_link_libraries(mametiny mame_tiny frontend optional emu formats dasm netlist utils expat softfloat softfloat3 wdlfft jpeg 7z asmjit lua lualibs linenoise zlib flac utf8proc sqlite3 ocore_headless -lm -lpthread -lutil -framework Foundation)
endif()

if(CMAKE_BUILD_TYPE MATCHES "headless-rel")
  add_library(mametiny_headless SHARED ${source_list})
  target_link_libraries(mametiny_headless mame_tiny frontend optional emu formats dasm netlist utils expat softfloat softfloat3 wdlfft jpeg 7z asmjit lua lualibs linenoise zlib flac utf8proc sqlite3 ocore_headless -lm -lpthread -lutil)
endif()
