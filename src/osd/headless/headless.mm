...
class headless_osd_interface final : public osd_interface, osd_output
{
public:
	enum osd_state
	{
	...
	auto set_buffer(void *buffer, osd_dim size) -> void;
	...
	// midi interface
	std::unique_ptr<osd_midi_device> create_midi_device() override
	{ return nullptr; };
...
- (instancetype)init
{
	...
	_osd = new headless_osd_interface(*opt);
...
- (NSUInteger)stateSize
{
	...
	return static_cast<NSUInteger>(ram_state::get_size(_machine->save()));
}
...
- (NSData *)serializeState
{
	...
	auto sz = ram_state::get_size(_machine->save());
	void *buf = malloc(sz);
	auto res = _machine->save().write_buffer(buf, sz);
	if (res != STATERR_NONE)
	{
		free(buf);
		return nil;
...
- (BOOL)deserializeState:(NSData *)data
{
	...
	auto res = _machine->save().read_buffer(const_cast<void *>(data.bytes), data.length);
	return static_cast<BOOL>(res == STATERR_NONE);
}
...
- (BOOL)_initializeGame:(NSError **)error
{
	...
	_config = new machine_config(*_gameDriver, *_options.options);
	_machine = new running_machine(*_config, *_manager);
	...
	_supportsSave = static_cast<BOOL>((_machine->system().flags & MACHINE_SUPPORTS_SAVE) != 0);

	return static_cast<BOOL>(_machine->phase() == machine_phase::RUNNING);
}
...
- (NSString *)name
{
	return NSSTRING_NO_COPY(_device->name().c_str());
}
...
- (instancetype)initWithDevice:(std::add_pointer_t<device_type> const)device
{
	if ((self = [super init]))
	{
		_name = @(device->fullname());
...
- (instancetype)initFromRecord:(media_auditor::audit_record const *)record
{
	if ((self = [super init]))
	{
		...
		if (shared_device)
		{
			_sharedDevice = [[Device alloc] initWithDevice:shared_device];
		}
	}
